...
def evalsets():
    ...
    这张图要证的是上册 §13.2 那个主张——**验收集就是需求文档本身最可执行的版本**。
...
def semantica():
    ...
    y0 = 126
    t(d, PAD, y0, "七要素　上册第 5 章那把尺子", 21, True, MUTED)
    ...
    t(d, PAD + 260, yb + 346,
      "按上册 §5.3 的硬规矩，缺了动作就不算业务本体——所以“开源版 Palantir”"
      "这个说法要打折扣。正确的用法：把它当知识层与证据层，动作层和权限层你自己在它外面接。",
      21, False, INK2)
    stamp(d, h=HE, text="依据下册 §5.4 · 对某一提交的判断")
